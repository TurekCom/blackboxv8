...
def analyze(path):
    ...
    cent=[]
    for f in frames:
        F=np.fft.rfft(f*np.hanning(len(f))); mag=np.abs(F); freqs=np.fft.rfftfreq(len(f),1/sr)
        cent.append(float((freqs*mag).sum()/(mag.sum()+1e-12)))
    cent=np.array(cent)
    f0s=[]; min_f,max_f=60,380; min_lag,max_lag=int(sr/max_f),int(sr/min_f)
...
paths = [
 ...
 Path('test_outputs/sapi5_x64_c64_question.wav'),
 Path('test_outputs/sapi5_x64_c64_exclaim.wav'),
 Path('test_outputs/sapi5_x64_clear_cluster.wav'),
]
for p in paths:
    if p.exists():
